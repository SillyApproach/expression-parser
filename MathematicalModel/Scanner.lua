dofile("./MathematicalModel/Token.lua")
...
function _scanner:scanToken()
    ...
    elseif c == "-" then
        _scanner.addToken(self, TokenType.MINUS)
    elseif c == "*" then
        _scanner.addToken(self, TokenType.STAR)
    elseif c == "/" then
        _scanner.addToken(self, TokenType.SLASH)
    elseif c == "^" then
        _scanner.addToken(self, TokenType.CARET)
    elseif c:match("%s+") then
        local _, j = self.source:find("%s+", self.start)
        self.current = j + 1
    elseif c:match("%d") then
        local _, j, text, fraction = self.source:find("(%d+(%.?%d*))", self.start)
        self.current = j + 1

        if fraction == "." then
            c = _scanner.advance(self)
            _scanner.error(self, c)
        end

        _scanner.addToken(self, TokenType.NUMBER, tonumber(text))
