dofile("./MathematicalModel/Grammar/Expression.lua")
dofile("./MathematicalModel/Grammar/Binary.lua")
dofile("./MathematicalModel/Grammar/Unary.lua")
dofile("./MathematicalModel/Grammar/Group.lua")
dofile("./MathematicalModel/Grammar/Literal.lua")
dofile("./MathematicalModel/Grammar/Identifier.lua")
...
function _parser:term()
    ...
    while _parser.match(self, TokenType.MINUS, TokenType.PLUS) do
...
function _parser:factor()
    ...
    while _parser.match(self, TokenType.SLASH, TokenType.STAR) do
        local operator = _parser.previous(self)
...
function _parser:unary()
    if _parser.match(self, TokenType.MINUS) then
        local operator = _parser.previous(self)
        local right = _parser.unary(self)
        return Unary.new(operator, right)
    end

    return _parser.exponent(self)
end

function _parser:exponent()
    local expr = _parser.primary(self)

    while _parser.match(self, TokenType.CARET) do
        local operator = _parser.previous(self)
        local right = _parser.primary(self)
        expr = Binary.new(expr, operator, right)
    end
...
function _parser:primary()
    if _parser.match(self, TokenType.NUMBER) then
